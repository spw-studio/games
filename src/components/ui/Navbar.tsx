...
export function Navbar({ player, onOpenPlayerModal }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-brand-800/20 bg-brand-900 text-white shadow-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8 h-16 sm:h-20">
        ...
        >
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-brand-700 to-brand-950 border border-gold-400/40 shadow-inner group-hover:border-gold-300">
            <ChefHat className="h-6 w-6 text-gold-300 transition-transform group-hover:rotate-6" />
          </div>
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5">
              ...
              <span className="rounded bg-gold-500/20 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-widest text-gold-300 border border-gold-400/30">
                PRO
              </span>
            </div>
            <span className="text-[11px] font-light tracking-widest text-cream-200/70 uppercase">
      ...
      <div className="md:hidden flex items-center justify-around border-t border-brand-800/50 bg-brand-950 px-2 py-2">
        {navLinks.map((link) => {
          const Icon = link.icon;
          const isActive =
            link.href === '/'
